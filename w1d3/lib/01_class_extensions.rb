...
class String
  def caesar(shift)
    ...
    self.each_byte do |ascii|
      letter_pos = ascii - "a".ord #standardize alphabet
      #gives every letter a corresponding num from 0-26

      shifted_letter_pos = (letter_pos + shift) % 26

      encoded_string << ("a".ord + shifted_letter_pos).chr
      #reconvert to ascii value
    end
...
class Hash
  def difference(other_hash)
    ...
    self.keys.each { |key| difference_hash[key] = self[key] unless other_hash.key?(key) }


    other_hash.keys.each { |key| difference_hash[key] = other_hash[key] unless self.key?(key) }
